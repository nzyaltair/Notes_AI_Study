...
预训练语言模型（Pre-trained Language Model, PLM）利用大规模无标注语料进行自监督预训练，学习通用语言表示，再通过微调或提示适配下游任务。这一"先预训练，后微调"的范式在 2018 年后彻底改变了 NLP 的研究与应用格局，是从"特定任务模型"到"通用基础模型"转变的关键里程碑，也是 [[03_大语言模型核心架构/00_大语言模型核心架构_综述|大语言模型]] 时代的直接前驱。
...
| [[09_LLM生命周期与训练导航]] | 生命周期导航 | 数据、分词、预训练、后训练、推理增强与部署接口 |
...
6. **建立生命周期视角**：[[09_LLM生命周期与训练导航]] — 将语言模型连接到数据、后训练、推理与服务。
...
- **Transformer 与注意力机制**：是预训练语言模型的核心架构基础。参见 [[../../B_连接主义与深度学习/05_Transformer与注意力机制/00_Transformer与注意力机制_综述|Transformer 与注意力机制]]。
- **大语言模型核心架构**：是预训练语言模型规模化和能力涌现的延伸。参见 [[03_大语言模型核心架构/00_大语言模型核心架构_综述|大语言模型核心架构]]。
- **模型训练与对齐**：预训练流程的后训练与工程化扩展。参见 [[../06_模型训练与对齐/00_大模型训练与对齐_综述|模型训练与对齐]]。
- **自然语言处理**：是语言智能的任务层与历史背景；它与 PLM/LLM 交叉演化，不能理解为 LLM 的下属集合。参见 [[08_自然语言处理/00_自然语言处理_综述|自然语言处理]]。

...
| 07 | [[A_基础与范式/07_控制理论/00_控制理论_综述\|控制理论]] ★ | PID、LQR、MPC、卡尔曼滤波、动态系统 | 具身智能与机器人的数学基础 |
...
| 08 | [[B_连接主义与深度学习/08_表示学习/00_表示学习_综述\|表示学习]] ★ | Embedding、迁移学习、多模态对齐、解耦表示 | 所有AI的核心横切关注点 |
| 09 | [[B_连接主义与深度学习/09_神经架构搜索NAS/00_神经架构搜索NAS_综述\|神经架构搜索NAS]] ★ | DARTS、可微搜索、一次性NAS、LLM驱动 | 用AI设计AI的早期实践 |
...
| 05 | [[C_基础模型与通用智能/05_世界模型基础/00_世界模型基础_综述\|世界模型基础]] ★ | Dreamer、JEPA、视频世界模型、3D场景模型 | Agent的预测与规划基础 |
| 06 | [[C_基础模型与通用智能/06_行动基础模型/00_行动基础模型_综述\|行动基础模型]] ★ | VLA、Action Tokenization、通用策略模型、模仿学习 | 基础模型向行动扩展 |
| 07 | [[C_基础模型与通用智能/07_模型训练与对齐/00_大模型训练与对齐_综述\|模型训练与对齐]] | SFT、RLHF、DPO、GRPO、LoRA | 模型能力的关键锻造 |
| 08 | [[C_基础模型与通用智能/08_推理与思考/00_推理与思维链_综述\|推理与思考]] ★ | CoT、ToT、Test-time Compute、搜索推理、PRM | **AGI阶梯2：推理 ≠ 生成** |
| 09 | [[C_基础模型与通用智能/09_模型压缩与高效推理/00_大模型推理与优化_综述\|模型压缩与高效推理]] | KV-Cache、量化、蒸馏、剪枝、投机解码 | 模型效率与部署优化 |
...
| 04 | [[G_具身智能与物理智能/04_具身控制与执行/00_具身控制与执行_综述\|具身控制与执行]] ★ | MPC、阻抗控制、Sim2Real、扩散策略 | 想到与做到的最后一公里 |
...
1. **A 基础**：数学 → 优化 → 机器学习 → 符号主义AI → 概率ML → 控制理论
2. **B 连接主义**：深度学习 → Transformer → 自监督学习 → 表示学习 → NAS
...
6. **G 具身智能**：机器人学基础 → 感知 → 决策 → 控制 → VLA → 人形 → 具身Agent（终点）

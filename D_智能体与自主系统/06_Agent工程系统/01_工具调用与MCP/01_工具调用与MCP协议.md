...
工具调用（Tool Use / Function Calling）使大语言模型（LLM）能够调用外部API和工具，是从"聊天机器人"到 [[../01_Agent系统与框架|Agent]] 的关键能力跃迁。LLM不直接执行操作，而是根据用户意图选择合适的工具并生成结构化调用参数，由外部执行器执行后返回结果。

本文聚焦协议细节与实现方式；领域定位、历史脉络与学习路径见 [[00_工具调用与MCP_综述|工具调用与MCP综述]]。
...
- [[../01_Agent系统与框架|Agent系统]]：工具调用是Agent的核心能力
- [[../03_RAG与检索增强/00_RAG系统|RAG]]：RAG检索可作为工具被动态调用
- **API编排**：LLM根据需求选择和组合多个API
- **代码执行**：生成并执行代码进行计算验证
- **数据库查询**：自然语言转SQL并执行
- [[../02_工作流与编排/01_自动化工作流|自动化工作流]]：多工具组合完成复杂任务
- **Computer Use**：Agent操作浏览器、桌面应用
- **文件操作**：读写文件、目录管理

## 8. 与其他技术关系

- 工具调用是 [[../01_Agent系统与框架|Agent系统]] 的行动能力
- [[../06_结构化输出|结构化输出]] 是工具调用的基础（工具参数依赖结构化输出）
- MCP标准化了Agent与工具的连接
- [[../05_提示工程|提示工程]] 影响工具选择的准确性
- 工具调用是 [[../02_工作流与编排/01_自动化工作流|自动化工作流]] 的执行层核心
- [[../03_RAG与检索增强/00_RAG系统|RAG]] 可作为工具被Agent调用
...
- 前置：[[../05_提示工程|提示工程]]、[[00_工具调用与MCP_综述|工具调用与MCP综述]]
- 平级：[[../06_结构化输出|结构化输出]]、[[../01_Agent系统与框架|Agent 系统与框架]]
- 延伸：[[../02_工作流与编排/01_自动化工作流|自动化工作流]]、[[../../../K_AI工程化/03_推理工程/01_LLM Serving|LLM Serving]]

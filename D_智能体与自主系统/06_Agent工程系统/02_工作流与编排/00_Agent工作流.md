...
Agent工作流是 [[../01_Agent系统与框架|Agent系统]] 在实际工程中的流程设计，强调可落地、可扩展、可维护的架构模式。它将Agent的任务执行过程建模为结构化工作流，通过感知、规划、执行、记忆和反思层的协同，实现端到端的任务自动化。

Agent工作流关注的是**Agent层面的任务执行流程设计**，而 [[01_自动化工作流|自动化工作流]] 关注的是**系统层面的编排、部署和运维**。两者互补：Agent工作流定义"如何完成任务"，自动化工作流定义"如何可靠地运行和管理任务"。
...
| 组件 | 功能 | 关键技术 |
|------|------|---------|
| 感知层 | 解析用户输入，识别意图和实体 | NLU、意图分类、[[../06_结构化输出|结构化输出]] |
| 规划层 | 任务分解，生成执行计划 | [[../../../C_基础模型与通用智能/08_推理与思考/02_思维链与提示推理|思维链]]、思维树、Plan-and-Execute |
| 执行层 | 调用工具和API完成任务 | [[../01_工具调用与MCP/01_工具调用与MCP协议|工具调用]]、代码执行器、沙箱 |
| 记忆层 | 管理上下文和历史经验 | 短期窗口、[[../03_RAG与检索增强/04_向量数据库|向量数据库]]、记忆摘要 |
...
- Agent工作流以 [[../01_Agent系统与框架|Agent系统与框架]] 为基础
- [[../01_工具调用与MCP/01_工具调用与MCP协议|工具调用与MCP协议]] 提供执行层能力
- [[../03_RAG与检索增强/00_RAG系统|RAG]] 作为知识检索工具集成到工作流
- [[../05_提示工程|提示工程]] 影响规划层质量
- [[../../05_记忆与持续学习/01_Agent记忆|Agent记忆]] 为工作流提供上下文管理
- [[01_自动化工作流|自动化工作流]] 在系统层面编排和运维Agent工作流
- [[../06_结构化输出|结构化输出]] 确保工作流各步骤间数据可解析
...
- 前置：[[../01_Agent系统与框架|Agent系统与框架]]、[[../01_工具调用与MCP/01_工具调用与MCP协议|工具调用与MCP协议]]
- 平级：[[../../05_记忆与持续学习/01_Agent记忆|Agent记忆]]
- 延伸：[[01_自动化工作流|自动化工作流]]

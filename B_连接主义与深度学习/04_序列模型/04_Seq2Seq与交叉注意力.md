...
- **历史意义**：注意力机制是 Seq2Seq 向 Transformer 过渡的桥梁——从"加性/点积注意力"到"自注意力"，理解注意力为什么出现以及如何运作，是理解 Transformer 自注意力机制的关键前置知识。这里的注意力本质是**交叉注意力（Cross-Attention）**：解码器查询编码器，Query 与 Key/Value 来自不同序列。
...
| 2017 | Transformer | Vaswani et al. | 自注意力彻底取代 RNN+注意力组合，交叉注意力被保留为 Encoder-Decoder 连接方式 |
...
通过计算 Query 和所有 Key 的相似度（注意力分数），得到加权权重，再对 Value 加权求和得到上下文向量。这与 Transformer 自注意力的区别在于：这里 Query 来自解码器序列，Key/Value 来自编码器序列（跨序列），而自注意力中三者来自同一序列。
...
| **Scaled Dot-Product** | $e_{t,i} = s_t^T h_i / \sqrt{d}$ | 缩放防梯度爆炸，被 Transformer 采用为标准形式 | Transformer 2017 |

注意力从"Seq2Seq 中锦上添花的增强模块"演化为"Transformer 中唯一的序列混合算子"，是深度学习架构史上最重要的范式迁移之一：自注意力（Self-Attention）将交叉注意力的思想推广到序列内部，Query=Key=Value 均来自同一序列的所有位置。
...
无注意力的 Seq2Seq 将所有输入信息压缩到单个向量 $c = h_T^{\text{enc}}$ 中。对于长序列，这一瓶颈导致信息丢失严重——翻译质量随输入长度增加而急剧下降（超过 30 词后 BLEU 骤降）。

注意力机制的本质是：**放弃单一瓶颈，改为动态构建每个解码步骤所需的定制化上下文**。这是深度学习架构设计中的一个通用原则——"保留所有信息，按需查询"比"预先压缩"更有效，这一原则同样是 KV Cache、检索增强生成（RAG）等现代技术的思想源头。
...
- **与 Transformer 的关系**：Transformer 的自注意力可以理解为 "Query=Key=Value=同一序列的所有位置" 的特例；Encoder-Decoder Transformer 中的交叉注意力层直接继承了本文的 Query-来自解码器、Key/Value-来自编码器设计
...
- 平级：[[07_状态空间模型SSM]]
- 延伸：[[../08_Transformer与注意力机制/00_Transformer与注意力机制_综述|Transformer]]、[[../08_Transformer与注意力机制/01_注意力机制原理|注意力机制原理（自注意力与多头）]]

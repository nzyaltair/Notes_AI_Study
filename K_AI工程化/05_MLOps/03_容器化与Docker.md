...
- [[../../K_AI工程化/03_推理工程/10_云端与本地部署|云端与本地部署]] 依赖容器实现跨环境一致性
- [[04_监控与日志]] 需要收集容器内应用的指标和日志
- [[05_版本管理与回滚]] 通过镜像标签管理模型版本
- [[../../K_AI工程化/03_推理工程/09_模型格式与转换/09_ONNX模型导出/00_ONNX模型导出|ONNX 模型导出]] 模型通过容器部署到不同硬件环境
